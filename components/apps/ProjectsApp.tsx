...
export default function ProjectsApp({ onOpenExternalLink }: { onOpenExternalLink?: (href: string) => boolean }) {
    return (
        <div className="flex-1 overflow-y-auto" style={{ background: '#1e1e1e', fontFamily: SANS }}>
            <div className="px-6 py-5 flex flex-col gap-6">
                ...
                {PORTFOLIO.projects.map((p, i) => <ProjectCard key={i} p={p} onOpenExternalLink={onOpenExternalLink} />)}
            </div>
        </div>
    );
}

function ProjectCard({ p, onOpenExternalLink }: { p: Project; onOpenExternalLink?: (href: string) => boolean }) {
        ...
        <div className="rounded-xl overflow-hidden"
            ...
            <div className="px-6 py-5">
                    ...
                    <div className="flex items-center gap-2 shrink-0">
                        ...
                        {p.githubUrl && (
                            <a href={p.githubUrl} target="_blank" rel="noopener noreferrer"
                                className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-[11px] leading-[1.2] transition-all"
                                ...
                                onClick={(e) => {
                                    ...
                                    e.preventDefault();
                                    onOpenExternalLink(p.githubUrl!);
                                }}
                ...
                <div className="flex flex-wrap gap-2 mb-3.5">
                    {p.tech.split(',').map(t => (
                        <span key={t.trim()} className="text-[11px] px-2.5 py-1 rounded leading-[1.2]"
                ...
                {/* Summary */}
                <p className="text-[13px] leading-[1.75]" style={{ color: '#aaa' }}>{p.summary}</p>
